            ...
            for link in self.getProductLinks():
                ...
                if review_count <= expected_review:
                    ...
                    if locator.getCustomerName() in self.driver.page_source:
                        ...
                        temp_customer_name = customer_name[my_count]
                        db_search = cassandra_client.table_query(table_name=searchString, product_name=product_name, customer_name=temp_customer_name)
                        time.sleep(2)
                        ...
                        while count <= total_review_page:
                            ...
                            for i in self.getReviewDetailsForProduct():
                                ...
                                comment = i[1][0]
                                customer_name = i[2][0]
                            print("review count is ", review_count)
                            print("expected count is ", expected_review)
                            if len(ratings) > 0:
                                for i in range(0, len(ratings)):
                                    ...
                                    print(result)
                                    values = (result['product_name'], result['customer_name'], result['comments'], result['discount_percent'], result['price'], result['product_searched'][0:12], str(result['rating']))
                                    is_not_entered = True
                                    for j in range(len(main_list)):
                                        if values[1] in main_list[j][1]:
                                    ...
                                    if review_count == expected_review:
                                        bulk_data = pd.DataFrame(main_list)
                                        for k in range(len(bulk_data)):
                                            cassandra_client.insert_one_row_to_table(table_name=searchString,
                                                                                     values=tuple(bulk_data.iloc[k]))
                                            time.sleep(2)
